...
`watch` on NE 596 shows nF attempting login with the 532L command form and the NE rejecting it, while manual login via `UTL::PRIVATE, PROV!` / `UTL::PUBLIC!` succeeds.
...
- **hborreli-LR:** Provided the correct 532/530 security-level commands — enter with `UTL::PRIVATE, mode!` (mode = TEST/PROV/ADMN), log off with `UTL::PUBLIC!`.
...
- **Issue Type:** Support (bug) → feature enhancement
- **Process Group:** LAN
